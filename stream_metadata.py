...
class StreamMetadata:
    ...
    def run_ffmpeg_audio_monitor(self):
        ...
        def build_cmd(audio_device):
            ...
            # If only audio monitor is enabled, just output to device
            if ENABLE_AUDIO_MONITOR and not ENABLE_AUDIO_METRICS:
                cmd.extend([
                    '-f', audio_device,
                    'default',
                ])
            # If audio metrics is enabled (with or without audio monitor), use filter graph
            elif ENABLE_AUDIO_METRICS:
        ...
        for device in ['pulse', 'alsa']:
            ...
            try:
                ...
                if self.ffmpeg_audio_process.poll() is not None and self.ffmpeg_audio_process.returncode != 0:
                    logging.warning(f"FFmpeg failed to start with device {device}, trying next device...")
                    continue
    ...
    def run(self):
        ...
        logging.info(f"Metadata: {metadata_status}")
